...
# --- SUPABASE SERVER CONFIG (service role key) ---
SUPABASE_URL = os.getenv("SUPABASE_URL")
SUPABASE_KEY = os.getenv("SUPABASE_KEY")  # **SERVICE ROLE** for server side
if not SUPABASE_URL or not SUPABASE_KEY:
    raise RuntimeError("Please set SUPABASE_URL and SUPABASE_KEY env variables")

supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
BUCKET = "images"  # create this bucket in Supabase storage and make public (or sign URLs)

@app.route("/", methods=["GET"])
def home():
    return jsonify({"status": "ok", "message": "Xaze Try-On Backend Running 🚀"})

# Optional: accept multipart upload for user images (client can use this instead of client-side upload)
...
def upload_user_image():
    try:
        ...
        # supabase-py returns dict on error. We check minimal.
...
# Main try-on endpoint: simple compositing demo (no GANs/Gemini) — reliable for hackathon
...
def tryon():
    try:
        ...
        # Simple composite algorithm:
        # scale cloth to ~70% of user width and paste around upper body
        ...
        # build mask: try alpha channel first, otherwise convert to grayscale and threshold
        ...
        out_buf.seek(0)
        file_bytes = out_buf.read()

        file_name = f"tryon_results/{user_id}_{product_id}_{int(time.time())}.png"
        upload_res = supabase.storage.from_(BUCKET).upload(file_name, file_bytes, {"upsert": True})
        if isinstance(upload_res, dict) and upload_res.get("error"):
            return jsonify({"status":"error","message":upload_res.get("error")}), 500
